...
export function RecruitPanel() {
  ...
  const html = useMemo(() => recruitmentEmail({ firstName, lang }).html, [firstName, lang]);
  const text = useMemo(() => recruitmentText({ firstName, lang }), [firstName, lang]);

  const send = () =>
    start(async () => {
      setErr(null);
      setSent(false);
      const res = await sendRecruitmentEmail(locale, { to, firstName, lang });

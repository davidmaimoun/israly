...
export type RecruitInput = {
  firstName: string;
  lang: "fr" | "en" | "he";
  customText?: string;   // accroche perso, insérée après la présentation
  linkedin?: string;
  portfolio?: string;    // "voir mon travail"
  siteUrl?: string;      // lien du site (invitation)
};

const RE: Record<"fr" | "en" | "he", {
  subject: string; hi: (n: string) => string; self: string; linkedinLabel: string; workLabel: string;
  intro: string; handles: string; founding: string; bullets: string[]; split: string; terms: string;
  siteIntro: string; siteBtn: string; cta: string; bye: string;
}> = {
  fr: {
    subject: "Israly — devenez guide fondateur",
    hi: (n) => `Bonjour ${n},`,
    self: "Je suis David Maimoun, créateur de sites web — je conçois et gère des plateformes comme celle-ci.",
    linkedinLabel: "LinkedIn", workLabel: "Voir mon travail",
    ...
    siteIntro: "Découvrez la plateforme et le rendu d'un profil guide :",
    siteBtn: "Voir le site",
  ...
  en: {
    ...
    hi: (n) => `Hi ${n},`,
    self: "I'm David Maimoun, a web developer — I design and run platforms like this one.",
    linkedinLabel: "LinkedIn", workLabel: "See my work",
    ...
    siteIntro: "Discover the platform and how a guide profile looks:",
    siteBtn: "Visit the site",
  ...
  he: {
    ...
    hi: (n) => `שלום ${n},`,
    self: "אני דוד מיימון, מפתח אתרים — אני בונה ומנהל פלטפורמות כמו זו.",
    linkedinLabel: "LinkedIn", workLabel: "העבודות שלי",
    ...
    siteIntro: "הכירו את הפלטפורמה ואת האופן שבו נראה פרופיל מדריך:",
    siteBtn: "לאתר",
...
export function recruitmentEmail(d: RecruitInput): { subject: string; html: string } {
  ...
  const links: string[] = [];
  if (d.linkedin) links.push(`<a href="${d.linkedin}" style="color:${C.blue};text-decoration:none;font-weight:bold">${t.linkedinLabel}</a>`);
  if (d.portfolio) links.push(`<a href="${d.portfolio}" style="color:${C.blue};text-decoration:none;font-weight:bold">${t.workLabel}</a>`);
  const linksLine = links.length ? `<p style="margin:0 0 14px;font-size:14px;color:${C.soft}">${links.join(" &nbsp;·&nbsp; ")}</p>` : "";
  const custom = d.customText && d.customText.trim()
    ? `<p style="margin:0 0 14px;font-size:15px;color:${C.ink}">${esc(d.customText.trim()).replace(/\n/g, "<br>")}</p>` : "";
  const siteBlock = d.siteUrl
    ? `<p style="margin:18px 0 8px;font-size:14px;color:${C.soft}">${t.siteIntro}</p>${btn(d.siteUrl, t.siteBtn)}` : "";
  const inner = `
    <p style="margin:0 0 6px;font-size:16px">${t.hi(d.firstName || "")}</p>
    <p style="margin:0 0 6px;font-size:14px;color:${C.ink}">${t.self}</p>
    ${linksLine}
    ${custom}
    ...
    <p style="margin:0 0 6px;font-size:14px;color:${C.soft}">${t.terms}</p>
    ${siteBlock}
    <p style="margin:18px 0 18px;font-size:14px;color:${C.ink}">${t.cta}</p>
...
export function recruitmentText(d: RecruitInput): string {
  const t = RE[d.lang] ?? RE.en;
  const lines = [t.hi(d.firstName || ""), t.self];
  if (d.linkedin) lines.push(`${t.linkedinLabel}: ${d.linkedin}`);
  if (d.portfolio) lines.push(`${t.workLabel}: ${d.portfolio}`);
  lines.push("");
  if (d.customText && d.customText.trim()) { lines.push(d.customText.trim(), ""); }
  lines.push(t.intro, "", t.handles, "", t.founding, ...t.bullets.map((b) => `• ${b}`), "", t.split, "", t.terms);
  if (d.siteUrl) lines.push("", `${t.siteIntro} ${d.siteUrl}`);
  lines.push("", t.cta, "", t.bye, "David — Israly");
  return lines.join("\n");
}
